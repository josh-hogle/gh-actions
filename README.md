...
[![license](https://img.shields.io/badge/license-apache-blue.svg)](https://github.com/josh-hogle/gh-actions/blob/master/LICENSE)
[![support](https://img.shields.io/badge/support-community-purple.svg)](https://github.com/josh-hogle/gh-actions)
[![Build Image](https://github.com/josh-hogle/gh-actions/workflows/Build%20Image/badge.svg?branch=releases/0.6.0)](https://github.com/josh-hogle/gh-actions/actions)
...
jobs:
    build:
        ...
        - id: set-conditionals
          name: Setup conditional variables
          uses: josh-hogle/gh-actions/set-conditional-vars@0.6.0
          with:
            log-message: |
                [
                    {
        ...
        - id: setup-vars
          ...
          if: |
            ...
            ${{ steps.set-conditionals.outputs.code_changed == 'true' }}) ||
            ${{ steps.set-conditionals.outputs.rebuild == 'true' }}
          uses: josh-hogle/gh-actions/set-docker-image-vars@0.6.0
          with:
            image-name: ${{ env.IMAGE_NAME }}
            base-image: ${{ env.BASE_IMAGE }}
            extra-tags: ${{ env.EXTRA_TAGS }}
        ...
        - id: pull-from-ghcr
          ...
          if: |
            ...
            ${{ steps.set-conditionals.outputs.code_changed == 'true' }}) ||
            ${{ steps.set-conditionals.outputs.rebuild == 'true' }}
          uses: josh-hogle/gh-actions/pull-docker-image@0.6.0
          with:
            image: ghcr://ghcr.io/josh-hogle/${{ env.PULL_FROM_GHCR }}
            username: ${{ github.actor }}
            password: ${{ secrets.GHCR_TOKEN }}
        ...
        - id: build-image
          ...
          if: |
            ...
            ${{ steps.set-conditionals.outputs.code_changed == 'true' }}) ||
            ${{ steps.set-conditionals.outputs.rebuild == 'true' }}
          uses: josh-hogle/gh-actions/build-docker-image@0.6.0
          with:
            image-name: ${{ steps.setup-vars.outputs.image-name }}
            base-image: ${{ steps.setup-vars.outputs.base-image }}
            version: ${{ steps.setup-vars.outputs.version }}
        ...
        - id: push-image
          ...
          if: |
            ...
            ${{ steps.set-conditionals.outputs.code_changed == 'true' }}) ||
            ${{ steps.set-conditionals.outputs.rebuild == 'true' }}
          uses: josh-hogle/gh-actions/push-docker-image@0.6.0
          with:
        ...
        - id: create-release
          ...
          if: |
            ...
            ${{ steps.set-conditionals.outputs.code_changed == 'true' }}) ||
            ${{ steps.set-conditionals.outputs.rebuild == 'true' }}
          uses: josh-hogle/gh-actions/create-release-with-assets@0.6.0
          with:
            access-token: ${{ secrets.GITHUB_TOKEN }}
            release-name: Release ${{ steps.setup-vars.outputs.version }}
```
